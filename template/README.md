...
Direktori `8001-boilerplate` ini adalah *template* standar yang digunakan untuk membuat proyek-proyek web (terutama Laravel) baru agar memiliki arsitektur, lingkungan, dan *resource usage* yang seragam. 
...
3. **Network Terpusat (`phantasm-net`)**: Proyek tidak mengurus *database* sendiri, melainkan menyambung ke jaringan infrastruktur pusat.
...
1. **Copy Direktori Ini**
   ```bash
   cp -r 8001-boilerplate 8003-toko-online
   cd 8003-toko-online
   ```
...
3. **Ubah Port & Nama Kontainer**
   ...
   - *Port mapping* Nginx. Berdasarkan standar kita, port web menggunakan awalan `8XXX`. Ubah `"8001:80"` menjadi `"8003:80"`.
